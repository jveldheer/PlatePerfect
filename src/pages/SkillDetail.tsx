    ...
    <div className="max-w-4xl mx-auto px-2 sm:px-0">
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 md:p-8 mb-4 sm:mb-6">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
          <span className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${getDifficultyColor(skill.difficulty)}`}>
            {skill.difficulty}
          </span>
          <span className="px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium bg-primary-100 text-primary-800">
        ...
        <div className="space-y-4 sm:space-y-6">
          {skill.detailedSteps.map((step, index) => (
            <div key={index} className="flex">
              <div className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 bg-primary-600 text-white rounded-full flex items-center justify-center font-bold text-sm sm:text-base mr-3 sm:mr-4">
                {index + 1}
              </div>
              <div className="flex-1">
                <p className="text-sm sm:text-base text-gray-800 leading-relaxed">{step}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Tips */}
      <div className="bg-green-50 rounded-xl shadow-lg p-4 sm:p-6 md:p-8 mb-4 sm:mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 flex items-center">
          <span className="text-2xl sm:text-3xl mr-2 sm:mr-3">💡</span>
          Pro Tips
        </h2>
      ...
      {/* Common Mistakes */}
      <div className="bg-red-50 rounded-xl shadow-lg p-4 sm:p-6 md:p-8 mb-4 sm:mb-6">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 flex items-center">
        ...
        <ul className="space-y-3">
          {skill.commonMistakes.map((mistake, index) => (
            <li key={index} className="flex items-start">
              <svg className="w-5 h-5 sm:w-6 sm:h-6 text-red-600 mr-2 sm:mr-3 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
              </svg>
              <span className="text-sm sm:text-base text-gray-800">{mistake}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="bg-primary-50 rounded-xl shadow-lg p-4 sm:p-6 md:p-8">
        ...
        <Link to="/recipes" className="btn-primary min-h-[44px] inline-flex items-center touch-manipulation">
          Browse Recipes
        </Link>
      </div>
    </div>
